...
int GUIAction::screenshot(std::string arg __unused)
{
	...
	if (PartitionManager.Is_Mounted_By_Path(storage)) {
		snprintf(path, sizeof(path), "%s/TWRP/Screenshots/", storage.c_str());
	} else {
		strcpy(path, "/tmp/");
	}
	...
	// Screenshot_Dark_2014-01-01-18-21-38.png
	strftime(path+path_len, sizeof(path)-path_len, "Screenshot_Dark_%Y-%m-%d-%H-%M-%S.png", localtime(&tm));
